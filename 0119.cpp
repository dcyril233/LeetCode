#ifdef LEETCODE
#include <LeetCodeL.hpp>
#endif

class Solution {
    long long getFactoria(int low, int high) {
        long long factoria = 1;
        for (long long i = low; i < high+1; ++i)
            factoria = factoria * i;
        return factoria;
    }
public:
    vector<int> getRow0(int rowIndex) {
        vector<int> v (rowIndex+1, 1);
        if (rowIndex > 1) {
            for (int i = 1; i < rowIndex/2+1; ++i) {
                long long a = getFactoria(rowIndex-i+1, rowIndex);
                long long b = getFactoria(1, i);
                long long value = a / b;
                v[i] = v[rowIndex-i] = value;
            }
        }
        return v;
    }
    vector<vector<int>> generate(int numRows) {
        vector<vector<int>> v;
        if (numRows == 0)
            return v;
        v.resize(numRows);
        for (int i = 0; i < numRows; ++i) {
            v[i].resize(i+1);
            if (i == 0) 
                v[0][0] = 1;
            else if (i == 1) {
                v[1][0] = 1;
                v[1][1] = 1;
            }
            else {
                v[i][0] = 1;
                for (int j = 1; j < i; ++j) {
                    v[i][j] = v[i-1][j-1] + v[i-1][j];
                }
                v[i][i] = 1;
            }
        }
        return v;
    }
    vector<int> getRow1(int rowIndex) {
        return generate(rowIndex+1)[rowIndex];
    }
    vector<int> getRow(int rowIndex) {
        vector<int> v (rowIndex+1, 1);
        if (rowIndex > 1) {
            for (long long i = 1; i < rowIndex/2+1; ++i) {
                v[i] = v[rowIndex-i] = v[i-1]*(rowIndex-i+1)/i;
            }
        }
        return v;
    }
};

#ifdef LEETCODE
int main() {
    // cout << " 1:" << endl;
    // {
    //     int rowIndex = 4;
    //     cout << Solution().getRow(rowIndex) << endl;
    // }
    cout << " 2:" << endl;
    {
        int rowIndex = 18;
        cout << Solution().getRow(rowIndex) << endl;
    }
    return 0;
}
#endif