...
public:
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
        ...
        int step = 0, count = 1;
        for(int i = 1; i < k; ++i) {
            if(nums[i] <= nums[index[(step+count-1)%k]]) {
                index[(count+step)%k] = i;
                ++count;
            }
            else {
                int delnum = 0;
                for(int j = (step+count-1)%k; j >= (step%k); --j) {
        ...
        ans[0] = nums[index[step%k]];
        for(int i = 1; i <= nums.size() - k; ++i) {
            if(i > index[step%k]) {
                ++step;
                --count;
            }
            if(nums[i+k-1] <= nums[index[(step+count-1)%k]]) {
                index[(count+step)%k] = i+k-1;
                ++count;
            }
            else {
                int delnum = 0;
                for(int j = step+count-1; j >= step; --j) {
                    if(nums[index[j%k]] >= nums[i+k-1])
                        break;
                    ++delnum;
                }
                count -= delnum;
